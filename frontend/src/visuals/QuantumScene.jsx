...
export default function QuantumScene({
  ...
  return (
    <div className="relative h-full min-h-[760px] overflow-hidden rounded-lg bg-black/20">
      <Canvas camera={{ position: [0, 2.35, 6.3], fov: 54 }}>
      ...
      <div className="pointer-events-none absolute inset-0" style={{ perspective: "1200px" }}>
        <div className="absolute left-4 top-4 max-w-xs rounded-md border border-cyanline/25 bg-black/35 p-3 backdrop-blur-md">
        ...
        <div className="absolute inset-x-5 bottom-5 rounded-lg border border-cyanline/25 bg-[#070b18]/72 p-4 shadow-[0_28px_80px_rgba(0,0,0,.55)] backdrop-blur-md" style={{ transform: "rotateX(10deg)", transformOrigin: "bottom center" }}>
          ...
          <div className="mt-4 grid gap-2 md:grid-cols-6">
            {SIMULATION_STEPS.slice(0, 6).map((step, index) => {
              ...
              return (
                <div key={step.id} className={active ? "rounded-md border border-cyanline/70 bg-cyanline/10 p-2 shadow-neon" : complete ? "rounded-md border border-emerald-300/40 bg-emerald-300/10 p-2" : "rounded-md border border-white/10 bg-black/20 p-2 opacity-70"}>
                  <div className="flex items-center gap-2">
                    <span className="grid h-7 w-7 place-items-center rounded bg-white/[0.07] font-display text-xs text-cyanline">{step.symbol}</span>
